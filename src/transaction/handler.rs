...
pub async fn download_csv(
    ...
    for record in &data {
        wtr.serialize(record).map_err(|e| AppError::Internal(e.to_string()))?;
    }
...
pub async fn download_json(
    ...
    headers.insert(
        header::CONTENT_DISPOSITION,
        "attachment; filename=\"transactions.json\"".parse().unwrap(),
    );
...
pub async fn download_xlsx(
    ...
    // Header row
    let headers = [
        "id", "external_id", "ida", "amount", "currency_code",
        "description", "mcc", "hold", "transaction_time",
        "receipt_id", "balance", "masked_pan", "mcc_description", "currency",
    ];
    ...
    for (row, t) in data.iter().enumerate() {
        ...
        sheet.write_string(row, 0, &t.id).map_err(|e| AppError::Internal(e.to_string()))?;
        sheet.write_string(row, 1, &t.external_id).map_err(|e| AppError::Internal(e.to_string()))?;
        sheet.write_number(row, 2, t.ida as f64).map_err(|e| AppError::Internal(e.to_string()))?;
        sheet.write_number(row, 3, t.amount as f64).map_err(|e| AppError::Internal(e.to_string()))?;
        sheet.write_number(row, 4, t.currency_code as f64).map_err(|e| AppError::Internal(e.to_string()))?;
        sheet.write_string(row, 5, t.description.as_deref().unwrap_or("")).map_err(|e| AppError::Internal(e.to_string()))?;
        if let Some(mcc) = t.mcc {
            sheet.write_number(row, 6, mcc as f64).map_err(|e| AppError::Internal(e.to_string()))?;
        }
        if let Some(hold) = t.hold {
            sheet.write_boolean(row, 7, hold).map_err(|e| AppError::Internal(e.to_string()))?;
        }
        sheet.write_string(row, 8, &t.transaction_time.to_rfc3339()).map_err(|e| AppError::Internal(e.to_string()))?;
        sheet.write_string(row, 9, t.receipt_id.as_deref().unwrap_or("")).map_err(|e| AppError::Internal(e.to_string()))?;
        if let Some(balance) = t.balance {
            sheet.write_number(row, 10, balance as f64).map_err(|e| AppError::Internal(e.to_string()))?;
        }
        sheet.write_string(row, 11, t.masked_pan.as_deref().unwrap_or("")).map_err(|e| AppError::Internal(e.to_string()))?;
        sheet.write_string(row, 12, t.mcc_description.as_deref().unwrap_or("")).map_err(|e| AppError::Internal(e.to_string()))?;
        sheet.write_string(row, 13, t.currency.as_deref().unwrap_or("")).map_err(|e| AppError::Internal(e.to_string()))?;
    }
    ...
    headers.insert(
        header::CONTENT_DISPOSITION,
        "attachment; filename=\"transactions.xlsx\"".parse().unwrap(),
    );
